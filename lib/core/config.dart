...
class AppConfig {
  ...
  static const String defaultSupabasePublishableKey =
  ...
  static Future<AppConfig> load() async {
    ...
    var key = await storage.getPublishableKey();
    if ((url ?? '').trim().isNotEmpty && (key ?? '').trim().isNotEmpty) {
      return AppConfig(supabaseUrl: url!.trim(), supabasePublishableKey: key!.trim());
    }
    // 2. Dart-define (build-time, e.g. CI/production)
    url = const String.fromEnvironment('SUPABASE_URL', defaultValue: '');
    key = const String.fromEnvironment('PUBLISHABLE_KEY', defaultValue: '');
